...
![](https://hackmd.io/_uploads/Bkp8a5fKF.png)
...
![](https://hackmd.io/_uploads/Sk8ntwJiO.png)
...
Data is stored at the leaf index corresponding to its value. E.g. if I have a sparse tree that can contain 2^256 values and want to prove non membership of the value 2^128. I can prove via a merkle membership proof that $tree\_values[2^{128}] = 0$, conversely if I can prove that $tree\_values[2^{128}] = 2^{128}$ I can prove that the item exists.
...
1. Look for a nullifier's corresponding low*nullifier, where $low_nullifier*{\textsf{next_value}} > v$.
2. Perform membership check of the low nullifier.
3. Perform a range check on the low nullifier's value and next*value fields:
   $new_nullifier > low_nullifier*{\textsf{value}}$ && ( $new\_nullifier <  low\_nullifier_{\textsf{next_value}}$ || $low\_nullifier_{\textsf{next_value}} == 0$ )
4. Update the low nullifier pointers

- $low\_nullifier_{\textsf{next_index}} = new\_insertion\_index$
- $low\_nullifier_{\textsf{next_value}} = new\_nullifier$

5. Perform insertion of new updated low nullifier (yields new root)
6. Update pointers on new leaf

- $new\_nullifier\_leaf_{\textsf{value}} = new\_nullifier$
- $new\_nullifier\_leaf.next\_value = low\_nullifier.next\_value$ (from before update in step 4)
- $new\_nullifier\_leaf_{\textsf{next_index}} = low\_nullifier_{\textsf{next_index}}$ (from before update in step 4)
...
Protocol without optimistic insertion:
...
let empty_subtree_hash = SOME_CONSTANT_EMPTY_SUBTREE;
let pending_insertion_subtree = [];
let insertion_index = inputs.next_insertion_index;
let root = inputs.current_nullifier_tree_root;
...
for (i in len(new_nullifiers)) {
    let new_nullifier = inputs.new_nullifiers[i];
    let low_nullifier_leaf_preimage = inputs.low_nullifier_leaf_preimages[i];
    let low_nullifier_membership_witness = inputs.low_nullifier_membership_witnesses[i];
...
From looking at the code above we can probably deduce why we need optimistic insertion. If the low nullifier does not yet exist in the tree, all of our membership checks will fail, we cannot produce a non membership proof.

To perform "optimistic" insertions, our circuit must keep track of all values that are pending insertion.

- If the `low_nullifier_membership_witness` is identified to be nonsense ( all zeros, or has a leaf index of -1 ) we will know that this is an "optimistic" low nullifier read request and we will have to look within our "pending subtree" for the nearest low nullifier.
...
for (i in len(new_nullifiers)) {
    ...
    if (low_nullifier_membership_witness is garbage) {
        ...
        // Search for the low nullifier "optimistically" within our pending insertion subtree
        for (j in range(0, i)) {
            let pending_nullifier = pending_insertion_subtree[j];
