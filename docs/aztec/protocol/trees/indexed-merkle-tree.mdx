...
1. Look for a nullifier's corresponding low_nullifier, where $low_nullifier\_{\textsf{next_value}} > v$.
...
3. Perform a range check on the low nullifier's value and next_value fields:
   $new_nullifier > low_nullifier\_{\textsf{value}}$ && ( $new_nullifier < low_nullifier*{\textsf{next*value}}$ || $low_nullifier*{\textsf{next_value}} == 0$ )
